...
Production checks found the segment wildcard did not cover root or paths ending in a slash. The legacy-domain redirects use an explicit all-path capture so those URLs also redirect while preserving the original path and query string.
