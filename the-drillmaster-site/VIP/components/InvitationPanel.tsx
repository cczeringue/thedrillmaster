...
import { ArrowLeft, Info, Ticket, Users, X } from "lucide-react";

type InvitationPanelProps = {
  open: boolean;
  section: "details" | "cast";
...
export function InvitationPanel({ open, section, introPlaying, hasReceipt, onOpenChange, onCloseAutoFocus, onRsvp, children }: InvitationPanelProps) {
  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}>
    <Dialog.Portal>
      ...
      <Dialog.Content className="info-panel" id="invitation-info-panel" onOpenAutoFocus={event => {
        ...
        <header className="info-panel__header">
          <div>
            <Dialog.Title className="info-panel__title" ref={titleRef} tabIndex={-1}>
              {section === "cast" ? <Users size={21} aria-hidden="true" /> : <Info size={21} aria-hidden="true" />}
        ...
        <div className="info-panel__body" tabIndex={0} aria-label={section === "cast" ? "Cast and roles" : "Event details"}>{children}</div>
