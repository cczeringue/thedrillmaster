...
The invite list is the Google Sheet [The Drillmaster - VIP Invite List - October 13, 2026](https://docs.google.com/spreadsheets/d/1GcwjfXeuGiZPxxuWmw8xuKBf6QO2zgYG6a8ZkSNkPKo/edit), in the ChatGPT folder. The `RSVPs` tab has Name, Email, Tickets needed, Received at, and RSVP reference columns. Caleb chose to retain its existing “Anyone with the link can edit” sharing on September 16, 2026. Do not change that sharing without his authorization.
...
The writer opens only the fixed RSVP sheet, validates fields and ticket limits, escapes spreadsheet formula prefixes, and uses a script lock plus the RSVP UUID to avoid duplicate rows on retries. Its GET endpoint cannot list RSVPs. Google Drive sharing for the sheet is managed separately from the endpoint. Apps Script's web app runs as the owner so guests do not need Google accounts; the application token protects the write endpoint.
